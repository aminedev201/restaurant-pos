      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          tr:hover { background-color: ${primaryColors[100]}; }
          .index { color: #6b7280; font-weight: 500; }
          .name { font-weight: 600; color: #111827; }
          .description { color: #4b5563; }
          .date { color: #6b7280; }
          @media print { body { padding: 0; } tr:hover { background-color: inherit; } }
        </style>
      </head>
      <body>
        <div class="header">
          ...
          <tbody>
            ${filteredCategories
              ...
              <tr>
                ...
                <td class="description">${cat.description || 'No description'}</td>
                <td class="date">${new Date(cat.created_at).toLocaleDateString()}</td>
              </tr>
            `
              )
              .join('')}
            ...
            <div className="flex flex-wrap items-center gap-2">
              {selectedCategories.length > 0 && (
                ...
                >
                  ...
                  <span className="hidden xs:inline">Delete ({selectedCategories.length})</span>
                  <span className="xs:hidden">({selectedCategories.length})</span>
                </button>
              )}

              <button
                onClick={() => setShowImportModal(true)}
                className="px-3 sm:px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition duration-200 font-medium text-xs sm:text-sm flex items-center gap-1 sm:gap-2"
              >
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute left-0 mt-2 w-40 sm:w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                      <button
                        ...
                        className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-t-lg"
                      >
                        JSON
                      </button>
                      <button
                        onClick={handleExportExcel}
                        className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-b-lg"
                      >
                        Excel
                      </button>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                      entries (Showing {indexOfFirstItem + 1}–{Math.min(indexOfLastItem, filteredCategories.length)} of{' '}
